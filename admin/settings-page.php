...
function course_certificate_admin_certificate_ui() {
	...
	if( isset($_POST['add_certificate']) ) {
		if(!wp_verify_nonce($_POST['course_nonce'], 'admin_certificate_ui')) {
		...
		} else if( empty($_POST['certificate_code']) || empty($_POST['std_name']) || empty($_POST['course_name']) || empty($_POST['course_duration']) || empty($_POST['award_date']) ) {
			...
			$editid = sanitize_text_field($_POST['editid']);
			$result = course_certificate_add_course_certificate($code, $name, $course, $duration, $award_date, $editid);
			if( $result == 1 ) {
				if( $editid != "" ) {
	...
	global $wpdb;
    $certificates = $wpdb->get_results( "SELECT * FROM AB_course_certificates");
    ...
    // Modify the query to search by student name, course name, certificate code, etc.
    $certificates = $wpdb->get_results(
        $wpdb->prepare("SELECT * FROM AB_course_certificates WHERE student_name LIKE %s OR course_name LIKE %s OR certificate_code LIKE %s",
            "%$search_query%", "%$search_query%", "%$search_query%")
    );
    
    // Pagination logic
    $cpage = isset($_POST['pg']) ? intval($_POST['pg']) : 1;
    $offset = ($cpage - 1) * 10;
    
    // Slice the certificates array for pagination
    $certificatesNew = array_slice($certificates, $offset, 10);
    
    
    // Pagination links (Next/Previous)
    $total_pages = ceil(count($certificates) / 10);
    $prev_page = $cpage > 1 ? $cpage - 1 : 1;
    $next_page = $cpage < $total_pages ? $cpage + 1 : $total_pages;
	if (!current_user_can('manage_options')) return;
    ...
    if (isset($_POST['bulk_upload']) && isset($_FILES['csv_file'])) {
        if (!wp_verify_nonce($_POST['course_bulk_nonce'], 'admin_certificate_bulk_ui')) {
            $error = '<div class="alert alert-danger">Verification Failed! Please try again.</div>';
        } else {
            ...
            if (($handle = fopen($file, "r")) !== FALSE) {
                ...
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    ...
                    $certificate_code = sanitize_text_field($data[0]);
                    $student_name = sanitize_text_field($data[1]);
                    $course_name = sanitize_text_field($data[2]);
                    $course_duration = sanitize_text_field($data[3]);
                    $award_date = sanitize_text_field($data[4]);

                    $result = $wpdb->insert(
                        'AB_course_certificates',
                        [
                ...
                $error = "<div class='alert alert-success'>Bulk upload completed! Success: $success_count, Errors: $error_count.</div>";
            } else {
                $error = '<div class="alert alert-danger">Failed to open the CSV file. Please try again.</div>';
            }
        }
    }
	      ...
	      <tbody>
	        <?php foreach ($certificatesNew as $value) { ?>
	    	 	<tr>
	                ...
	                <td class="chour"><?php echo $value->course_duration; ?></td>
      ...
      <form class="mt-40" method="POST">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input type="text" required class="form-control" name="course_duration">
          </div>
          <div class="form-group">
            ...
            <input 
                ...
                value="" 
                <?php if (!current_user_can('administrator')) { echo 'readonly="readonly"'; } ?> 
                required
            >
...
<!-- Add Bulk Upload Form -->
<div class="container">
    <form method="POST" enctype="multipart/form-data">
        <?php wp_nonce_field('admin_certificate_bulk_ui', 'course_bulk_nonce'); ?>
        <div class="form-group">
            <label for="csv_file">Upload CSV File</label>
            <input type="file" class="form-control" name="csv_file" accept=".csv" required>
        </div>
        <button type="submit" name="bulk_upload" class="btn btn-primary">Upload CSV</button>
    </form>
</div>
...
if (isset($_POST['bulk_upload'])) {
    ...
    if (!isset($_POST['course_bulk_nonce']) || !wp_verify_nonce($_POST['course_bulk_nonce'], 'admin_certificate_bulk_ui')) {
    ...
    if (!empty($_FILES['csv_file']['tmp_name'])) {
        ...
        if (($handle = fopen($file, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle, 1000, ',')) !== false) {
                ...
                $course_name = sanitize_text_field($data[1]);
                $course_duration = sanitize_text_field($data[2]);
                $certificate_code = sanitize_text_field($data[3]);
                $award_date = sanitize_text_field($data[4]);
            ...
            echo '<div class="alert alert-success">Certificates have been uploaded successfully!</div>';
        } else {
            echo '<div class="alert alert-danger">Unable to open the uploaded file. Please try again.</div>';
        }
    } else {
        echo '<div class="alert alert-danger">No file was uploaded. Please try again.</div>';
    }
}

// Handle manual certificate addition
if (isset($_POST['add_certificate'])) {
    if (!isset($_POST['course_nonce']) || !wp_verify_nonce($_POST['course_nonce'], 'admin_certificate_ui')) {
        wp_die('Security check failed');
    }

    $student_name = sanitize_text_field($_POST['std_name']);
    ...
    echo '<div class="alert alert-success">Certificate added successfully!</div>';
}
?>
...
<form method="POST" action="">
    ...
    <!-- Pagination Buttons -->
    <?php if ($cpage > 1) : ?>
        <button type="button" onclick="submitForm(<?php echo $cpage - 1; ?>)">Previous</button>
    <?php endif; ?>
    <?php if ($cpage < $total_pages) : ?>
        <button type="button" onclick="submitForm(<?php echo $cpage + 1; ?>)">Next</button>
    <?php endif; ?>
</form>
...
<style>
    ...
    form {
        ...
        margin-top: 20px; /* Adjust to a reasonable top margin */
    }
...
<script>
    // Function to submit the form with the updated page number
    ...
    function resetForm() {
        ...
        resetInput.name = 'search_query';
        resetInput.value = ''; // Clear search query
        form.appendChild(resetInput);

        var pageInput = document.createElement('input');
        pageInput.type = 'hidden';
        pageInput.name = 'pg';
        pageInput.value = 1; // Reset to the first page
        form.appendChild(pageInput);
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {
			...
			jQuery(document).on("click", ".editModal", function() {
				...
				jQuery("#editEmployeeModal input[name=course_duration]").val(chour);
